    ...
    <div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <div className="rounded-xl border bg-white p-6 shadow-sm lg:col-span-2">
          ...
          >
            ...
            <Input label="URL Avatar" error={profileForm.formState.errors.avatarUrl?.message} {...profileForm.register('avatarUrl')} />
            {profileForm.watch('avatarUrl') && (
              <img src={profileForm.watch('avatarUrl')} alt="Avatar preview" className="h-20 w-20 rounded-full object-cover" />
            )}
        ...
        <div className="rounded-xl border bg-white p-6 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <div>
              <dt className="text-slate-500">Hạn thẻ</dt>
              <dd className="font-medium">{user?.cardExpiryDate ? new Date(user.cardExpiryDate).toLocaleDateString('vi-VN') : '—'}</dd>
            </div>
          </dl>
        </div>
      </div>
      ...
      <Modal open={showPasswordModal} onClose={() => setShowPasswordModal(false)} title="Đổi mật khẩu">
        <form
          ...
          className="space-y-4"
        >
          <Input label="Mật khẩu cũ" type="password" {...passwordForm.register('oldPassword')} />
          <Input label="Mật khẩu mới" type="password" {...passwordForm.register('newPassword')} />
          <Input label="Xác nhận mật khẩu mới" type="password" error={passwordForm.formState.errors.confirmPassword?.message} {...passwordForm.register('confirmPassword')} />
